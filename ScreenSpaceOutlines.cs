...
namespace SSOutlines
{
    public class ScreenSpaceOutlines : ScriptableRendererFeature
    {
        ...
        private class ScreenSpaceOutlinesPass_New : ScriptableRenderPass
        {
            ...
            public ScreenSpaceOutlinesPass_New(RenderPassEvent renderPassEvent, OutlineShaderMaterialSettings outlineShaderMaterialSettings, LayerMask layerMask, TemporalReprojection temporalReprojection)
            {
                this.renderPassEvent = renderPassEvent;
                this.temporalReprojection = temporalReprojection;
                
                Shader shader = Shader.Find("Shader Graphs/OutlineShader_v2");
            ...
            public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
            {
                ...
                //cmd.GetTemporaryRT(temporaryBufferID, desc.width, desc.height, 0, FilterMode.Bilinear, RenderTextureFormat.RGFloat, RenderTextureReadWrite.Default);
            ...
            public override void Configure(CommandBuffer cmd, RenderTextureDescriptor cameraTextureDescriptor)
            {
                ...
                textureDescriptor.colorFormat = RenderTextureFormat.RGFloat;
                textureDescriptor.depthBufferBits = 0;

                textureDescriptor.width = Mathf.RoundToInt(textureDescriptor.width);
                textureDescriptor.height = Mathf.RoundToInt(textureDescriptor.height);
                
                cmd.GetTemporaryRT(Shader.PropertyToID(sso.name), textureDescriptor, FilterMode.Bilinear);
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                using(new UnityEngine.Rendering.ProfilingScope(cmd, new ProfilingSampler("ScreenSpaceOutlines")))
                {
                    ...
                    // Create a full-screen quad (or any desired screen covering geometry)
                    

                    // Apply the shader/material
                    cmd.Blit(null, renderingData.cameraData.renderer.cameraColorTargetHandle, screenSpaceOutlineMaterial);
                    */     
                    ...
                    //cmd.Blit(renderingData.cameraData.renderer.cameraColorTargetHandle, temporaryBuffer);
                    cmd.Blit(null, temporaryBuffer, mat: screenSpaceOutlineMaterial);

                    //cmd.Blit(temporaryBuffer, renderingData.cameraData.renderer.cameraColorTargetHandle);

                    // Set the previous frame buffer as a global texture
                    cmd.SetGlobalTexture(temporaryBufferID, temporaryBuffer);
                    */
                    //cmd.SetRenderTarget(cameraColorTarget);
                    //cmd.ClearRenderTarget(true, true, Color.clear);

                    //UnityEngine.Matrix4x4 projectionMatrix_2 = renderingData.cameraData.camera.GetProjectionMatrix(-temporalReprojection._frustumJitter.activeSample.x, -temporalReprojection._frustumJitter.activeSample.y);
                    //cmd.SetProjectionMatrix(projectionMatrix);
                    ...
                    //cmd.SetProjectionMatrix( renderingData.cameraData.camera.GetProjectionMatrix());
                }
            ...
            public override void OnCameraCleanup(CommandBuffer cmd)
            {
                cmd.ReleaseTemporaryRT(Shader.PropertyToID(sso.name));
            }
        }
        ...
        private class NonMaximumSuppressionPass : ScriptableRenderPass
        {
            ...
            public const string USE_NMS = "USE_NMS";
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                if(outlineShaderMaterialSettings.useNMS)
                {
                    nmsMaterial.EnableKeyword(USE_NMS);
                }
                else
                {
                    nmsMaterial.DisableKeyword(USE_NMS);
                }
                ...
                using(new UnityEngine.Rendering.ProfilingScope(cmd, new ProfilingSampler("NMS_TAA")))
                {
                    ...
                    */
                    ...
                    // Ensure the render target is set to the screen before blitting
                    //cmd.SetRenderTarget(cameraColorTarget);

                    
                    if(temporalReprojection.useTAA)
                    {
                        //Blit(cmd, cameraColorTarget, cameraColorTarget, compositeMaterial);
                        
                        //Debug.Log("Taa pass");

                        //cmd.Blit()
                        // Blit the current camera color target to a temporary buffer
                        //Blit(cmd, cameraColorTarget, backBuffer1, nmsMaterial);
                        cmd.Blit(null, backBuffer1, nmsMaterial);

                        //cmd.Blit(backBuffer1, cameraColorTarget_);

                        //cmd.SetRenderTarget(hbuffer);
                        //cmd.ClearRenderTarget(true, true, Color.clear); 

                        //cmd.Blit(hbuffer, cameraColorTarget);

                        // Set the previous frame buffer as a global texture
                        //cmd.SetGlobalTexture(backBuffer1_ID, backBuffer1);
                            ...
                            //cmd.Blit(screen, cameraColorTarget);
                            ...
                            //cmd.Blit(backBuffer1, backBuffer2, TAAMaterial);
                            //cmd.Blit(backBuffer1, backBuffer2, TAAMaterial);
                    
                        //
                        //cmd.DrawRenderer(m_objToPaint, m_mat, 0, 0);
                        // Ensure the previous frame buffer is cleared before use
                        //cmd.SetRenderTarget(previousFrameBuffer);
                        //cmd.ClearRenderTarget(true, true, Color.clear);

                        

                        // Ensure the back buffer is cleared 
                        //cmd.SetRenderTarget(previousFrameBuffer);
                        //cmd.ClearRenderTarget(true, true, Color.clear);

                        // Apply the NMS material to the temporary buffer and blit it back to the camera color target
                        //Blit(cmd, backBuffer2, cameraColorTarget, compositeMaterial);
                        //Blit(cmd, backBuffer1, cameraColorTarget);
                        //cmd.Blit(backBuffer2, cameraColorTarget);
                        

                        // current outline frame buffer to previous frame buffer
                        //Blit(cmd, backBuffer2, previousFrameBuffer);

                        

                        // Ensure the back buffer is cleared 
                        //cmd.SetRenderTarget(backBuffer2);
                        //cmd.ClearRenderTarget(true, true, Color.clear);
        ...
        public override void Create()
        {
            ...
            screenSpaceOutlinesPass = new ScreenSpaceOutlinesPass_New(outlineRenderPassEvent, outlineShaderMaterialSettings,  outlinesLayerMask, temporalReprojection);
